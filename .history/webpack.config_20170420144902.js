...
let myproxy=new BrowserSyncPlugin({
    host: 'localhost',
    port: 9002,
    proxy: 'http://172.16.2.67:9000/', // 这里的proxy和webpack-dev-server中的proxy作用不一样，这里的proxy表示本地已经有一个server了，所以要代理一下（看有道笔记中关于browser-sync的文章），webpack-dev-server中的proxy是用来解决跨域问题的。
